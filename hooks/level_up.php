...
function hook_level_up($container, $args = 0)
{
    ...
    if ( $args->exp >= $args->max_exp )
    {
        ...
        //Update the database
        $container['db']->execute('UPDATE `<ezrpg>players_meta` SET `level`=level+1, `stat_points`=stat_points+2, `max_exp`=max_exp+20, `energy`=energy+1, `max_energy`=max_energy+1, `hp`=hp+5, `max_hp`=max_hp+5 WHERE `pid`=?', array( intval($args->id) ));
