...
class Module_Attack extends Base_Module
{
    ...
    public function start()
    {
        ...
        if (isset($_SERVER["HTTP_REFERER"]) && strpos($_SERVER["HTTP_REFERER"], 'mod=Members')) {
            $numquery = $this->db->execute('SELECT count(*) as num FROM `<ezrpg>attack_log` WHERE alog_attacker = ? AND alog_defender = ? AND alog_timestamp > NOW() - INTERVAL 24 HOUR',
                array($this->player->id, $_GET['p']));
    ...
    {
        $attmult = rand(7, 8);
        $attWeapons = $this->container['hooks']->run_hooks('attack_upgrades',
            array('type' => 'offense', 'player' => $attacker));
        $attstr = ($attacker->strength + $attWeapons) * $attmult;
        $attagil = $attacker->agility * $attmult;

        return array(
            'Score' => (($attstr) + ($attagil)) * $attmult,
            'Strength' => $attstr,
            'Agility' => $attagil,
            'Multi' => $attmult,
            'HP' => $attacker->hp,
    ...
    {
        $defmulti = rand(5, 7);
        $defWeapons = $this->container['hooks']->run_hooks('attack_upgrades',
            array('type' => 'defense', 'player' => $defender))['weapons'];
    ...
    private function getDamage($player, $victor = false)
    {
        if ($victor) {
            return array(
                'HP' => $player->hp - round(rand($player->hp / 6, $player->hp)),
                'XP' => ($player->exp != '0' ? $player->exp + round(rand($player->exp / 5, $player->exp / 2)) : rand(6,
                    12)),
                'Debug' => $player->exp
            );
        } else {
            return array(
                'HP' => $player->hp - rand($player->hp / 2, $player->hp + 10),
                'XP' => ($player->exp != '0' ? $player->exp + round(rand($player->exp / 6, $player->exp / 4)) : rand(3,
                    8)),
                'Debug' => $player->exp
            );
        }
