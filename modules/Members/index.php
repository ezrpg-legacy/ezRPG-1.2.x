...
class Module_Members extends Base_Module
{
    ...
    public function start()
    {
        ...
        $query = $this->db->execute('SELECT `<ezrpg>players`.`id`, `username`, `level` FROM `<ezrpg>players` INNER JOIN `<ezrpg>players_meta` ON `pid` = `<ezrpg>players`.`id` ORDER BY `<ezrpg>players`.`id` ASC LIMIT ?,50', array( $page * 50 ));
