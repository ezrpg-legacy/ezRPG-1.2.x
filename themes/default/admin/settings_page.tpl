		...
		<tr>
			<td valign="top">
				...
				{if $sitem->optionscode == 'radio'}
					{foreach from=$allSettings item=subitem}
					{if $sitem->id == $subitem->gid}
					<input name="sgid{$sitem->id}" type='radio' name='setting{$sitem->id}' value="{$subitem->id}" {if $subitem->id == $sitem->value} checked{/if}>{$subitem->title}</input>
					{/if}
					{/foreach}
				{/if}
			</td>
